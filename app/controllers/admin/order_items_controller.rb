class Admin::OrderItemsController < ApplicationController
  ...
  def update
    ...
    order_item_in_production_count = @order.order_items.where(product_status: OrderItem.product_statuses.key(2)).count
    order_item_production_complete_count = @order.order_items.where(product_status: OrderItem.product_statuses.key(3)).count
